...
function processAttendanceFile(filename, base64Data) {
  try {
    ...
    allSplitItems.forEach(splitItem => {
      ...
      if (isDisease) {
        const hasHospitalDoc = ["진료", "진단", "처방", "통원", "입원", "소견", "약", "영수증"].some(kw => splitItem.attachments && splitItem.attachments.includes(kw));
        const hasSubstituteDoc = ["학부모", "담임", "확인서", "의견서", "서명"].some(kw => splitItem.attachments && splitItem.attachments.includes(kw));
    ...
    generationConfig: {
      ...
      response_schema: {
        ...
        items: {
          ...
          required: ["date", "studentId", "name", "studentSigned", "category", "subCategory", "periodStart", "periodEnd", "totalDays", "reasonDetail", "parentName", "parentSigned", "teacherName", "teacherSigned", "attachments", "docStartDate", "docEndDate", "issuer", "ruleCheck", "neisString"]
        }
      }
    }
  };
...
function reconcileNeisWithAttendance(targetMonth, targetClass) {
  ...
  neisRows.forEach((row) => {
    ...
    mapping.forEach(m => {
      ...
      // If female, no exam in this month, subCategory is "인정":
      // 1) If NEIS value is higher than local value, we assume the undocumented recognized absences are menstrual pain and suppress the mismatch error.
      // 2) If NEIS value is lower than local value, but the difference is within the local menstrual records (tracked as statKey_menstrual), we also suppress it.
      if (isMismatch && m.sub === "인정" && isFemale && !hasExamInMonth) {
        const mKey = `${statKey}_menstrual`;
        ...
        if (neisVal > localVal) {
          isMismatch = false;
        } else if (neisVal < localVal && (localVal - neisVal) <= localMenstrualVal) {
          isMismatch = false;
        }
      }
...
function checkPreviousDayHospitalDoc(studentId, currentDateStr, currentBatchItems) {
  ...
  // Find the exact previous school day
  const prevDate = new Date(current);
  if (current.getDay() === 1) { // Monday -> Friday (-3 days)
    prevDate.setDate(current.getDate() - 3);
  } else if (current.getDay() === 0) { // Sunday -> Friday (-2 days)
    prevDate.setDate(current.getDate() - 2);
  } else {
    // Tue, Wed, Thu, Fri, Sat -> -1 day
    prevDate.setDate(current.getDate() - 1);
  }
  
  const prevDateStr = Utilities.formatDate(prevDate, "GMT+9", "yyyy-MM-dd");
  ...
  if (currentBatchItems && Array.isArray(currentBatchItems)) {
    for (let i = 0; i < currentBatchItems.length; i++) {
      ...
      if (rId === String(studentId).trim() && rCat.includes('질병')) {
        ...
        if (coveredDates.includes(prevDateStr)) {
          if (rAttach.includes('진료') || rAttach.includes('진단') || rAttach.includes('처방') || rAttach.includes('소견')) {
            return true;
          }
        }
      }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const rCat = String(row[4] || "").trim(); // 대분류 (질병, 인정, etc.)
    const rAttach = String(row[14] || "").trim();
    
    if (rId === String(studentId).trim() && rDate === prevDateStr && rCat.includes('질병')) {
      if (rAttach.includes('진료') || rAttach.includes('진단') || rAttach.includes('처방') || rAttach.includes('소견')) {
        return true;
      }
    }
  ...
  dataBody.forEach((row) => {
    ...
    mapping.forEach((m) => {
      ...
      if (isMismatch && m.sub === "인정" && isFemale && !hasExamInMonth) {
        ...
        if (neisVal > localVal) {
          isMismatch = false;
        } else if (neisVal < localVal && (localVal - neisVal) <= menstrualCount) {
          isMismatch = false;
        }
      }
